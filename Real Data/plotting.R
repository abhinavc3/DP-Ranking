...
plot_vs_p <- function(data, label_of_y) {
  ggplot(sim_summary(data), aes(x = p)) +
    ...
    ylab(label_of_y) +
    labs(color = latex2exp::TeX(r'($\epsilon$)')) +  # Change the color legend title here
    scale_shape_discrete(name = latex2exp::TeX(r'($\epsilon$)')) +  # Optional: Change the shape legend title
    scale_linetype_discrete(name = latex2exp::TeX(r'($\epsilon$)')) +
...
plot_vs_epsilon <- function(data, label_of_y) {
  ggplot(sim_summary(data), aes(x = epsilon)) +
    geom_point(aes(y = mean, shape = as.factor(p), colour = as.factor(p)), size = 3) +
    geom_line(aes(
      y = mean,
    ...
    labs(color = latex2exp::TeX(r'($p$)')) +  # Change the color legend title here
    scale_shape_discrete(name = latex2exp::TeX(r'($p$)')) +  # Optional: Change the shape legend title
    scale_linetype_discrete(name = latex2exp::TeX(r'($p$)')) +
