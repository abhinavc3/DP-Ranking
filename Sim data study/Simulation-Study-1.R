...
# Experiment 0
#vector = runif(10)
#obj_func(runif(10), win_mat = data_gen(), n = nrow(win_mat))

n <- 300
fixed_theta <- c(rnorm(n-k), rnorm(k, 5, 1))
...
top_k_borda_count <-
  function(win_mat, k = round(nrow(win_mat) / 3), eps = 0.1, lambda = 2/eps) {
...
data <- data_gen(n = n, theta = fixed_centered_theta, p = 1)
S_k = recover_top_k_set(fixed_centered_theta)
hat_S_k = top_k_borda_count(win_mat = data)
length(intersect(S_k, hat_S_k)) * 3 / n


#### Experiment 1
n_seq <- seq(100, 200, 50)
no_of_iter <- 100
hamming_error <- matrix(nrow = length(n_seq), ncol = no_of_iter)
hamming_error_priv <- matrix(nrow = length(n_seq), ncol = no_of_iter)
for (i in 1:length(n_seq)) {
  n <- n_seq[i]
  k <- round(n / 4)
  fixed_theta <- rnorm(n)
  ...
  for (iter in 1:no_of_iter) {
    ...
    hat_S_k = top_k_borda_count(win_mat = data, k = k, lambda = 0)
    hat_S_k_priv = top_k_borda_count(win_mat = data, k = k)
    hamming_error[i, iter] <- 1 - length(intersect(S_k, hat_S_k)) / k
    hamming_error_priv[i, iter] <- 1 - length(intersect(S_k, hat_S_k_priv)) / k
  }
  #avg_hamming_error[i] <- mean(hamming_error)
  #avg_hamming_error_priv[i] <- mean(hamming_error_priv)
  print(i)
}


plot_sim_results(hamming_error, hamming_error_priv, "Hamming Loss")
