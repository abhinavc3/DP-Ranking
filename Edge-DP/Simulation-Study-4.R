...
param_seq <- c(5, 4.25, 2.5, 1.9, 1.25, 0.75, 0.3, 0.1, 0)
no_of_iter <- 45
...
#hamming_error_count <-  matrix(nrow = length(param_seq), ncol = no_of_iter)
hamming_error_count_priv <-
  matrix(nrow = length(param_seq), ncol = no_of_iter)

writeLines(c(""), "log.txt")
for (i in 1:length(param_seq)) {
  epsilon_inv <- param_seq[i]
...
setwd(
  "C:/Users/abch/Dropbox (Penn)/Pairwise Comparison (Abhinav & Yichen)/Numerical-Study/"
)

write.csv(param_seq, "./Simulation-Results/accuracy-vs-epsilon/p=0.5 n=300/epsilon_inv_seq.csv")

write.csv(
  hamming_error_est_priv,
  "./Simulation-Results/accuracy-vs-epsilon/p=0.5 n=300/hamming-error-est-priv.csv"
)

write.csv(
  hamming_error_count_priv,
  "./Simulation-Results/accuracy-vs-epsilon/p=0.5 n=300/hamming-error-count-priv.csv"
)


write.csv(
  L_infty_error_priv,
  "./Simulation-Results/accuracy-vs-epsilon/p=0.5 n=300/L-infty-error-priv.csv"
)

############ Reading simulation results ########

param_seq <- read.csv("./Simulation-Results/accuracy-vs-epsilon/p=0.5 n=300/epsilon_inv_seq.csv")


hamming_error_est_priv <-
  read.csv("./Simulation-Results/accuracy-vs-epsilon/p=0.5 n=300/hamming-error-est-priv.csv")

hamming_error_count_priv <-
  read.csv("./Simulation-Results/accuracy-vs-epsilon/p=0.5 n=300/hamming-error-count-priv.csv")


L_infty_error_priv <-
  read.csv("./Simulation-Results/accuracy-vs-epsilon/p=0.5 n=300/L-infty-error-priv.csv")
...
ggsave(
  "./Plots/accuracy-vs-epsilon/p=0.5 n=300/hamming loss via estimation.pdf",
  plot_sim_results_single(
    hamming_error_est_priv[, -1],
    param_seq[, -1],
    "hamming loss via estimation",
    latex2exp::TeX(r'($1/\epsilon$)')
  ),
  height = 15,
  width = 12
)
...
ggsave(
  "./Plots/accuracy-vs-epsilon/p=0.5 n=300/hamming loss via counting.pdf",
  plot_sim_results_single(
...
ggsave(
  "./Plots/accuracy-vs-epsilon/p=0.5 n=300/l infinity loss.pdf",
  plot_sim_results_single(
    L_infty_error_priv[, -1],
    param_seq[, -1],
    latex2exp::TeX(r'(Relative $L_\infty$ loss)'),
    latex2exp::TeX(r'($1/\epsilon$)')
  ),
  height = 15,
  width = 12
